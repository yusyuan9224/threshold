...
Domain／Bluetooth／System／Diagnostics／Runtime／App 六層皆已實作，`swift test` 全綠，App 可組成 `Threshold.app` 執行，且已在真實裝置上完成一次完整的離開→鎖定→返回→喚醒循環驗證。**尚未達到無條件 v1.0**：簽章憑證與部分裝置矩陣仍在 repo 之外。
